...
;
	...
	;
	; Modify the program below to replace any uppercase characters with lowercase.
...
doGet
	...
	; A = 0x41, 	Z = 0x5A
	; to convert: do 'a' - 'A'
	; if userInput >= A and <= Z, SUB R0, R0, #('a' - 'A')	
	
	CMP 	R0, #'A'	; if R0 >=A
	BLO	endNow		; else, branch to end
	CMP	R0, #'Z'	; if R0 <=Z
	BHI	endNow		; else, branch to end
	ADD 	R0, R0, #('a' - 'A')	; convert uppercase to lowercase
endNow
	BL	put
	CMP	R0, #0x0D	; Stop when RETURN is entered (ASCII 0x0D)
	BNE	doGet
